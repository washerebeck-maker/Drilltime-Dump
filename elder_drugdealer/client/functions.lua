...
Helper.MakeEntityFaceEntity = function(entity1, entity2)
...
Helper.RandomFloat = function(lower, greater)
    return lower + math.random()  * (greater - lower);
end

Helper.NotifyError = function(text)
    ESX.ShowNotification(text)
end

Helper.NotifySuccess = function(text)
    ESX.ShowNotification(text)
end

Helper.GetRandomPositionInRadius = function()
    local angle = math.random() * 2 * math.pi
    return 10.0 * math.cos(angle), 10.0 * math.sin(angle) 
end

Helper.GetLabel = function(_item)
	local item  = exports.ox_inventory:Items(_item)
	return item and item.label or _item
end

Helper.ShowNotification = function(msg)

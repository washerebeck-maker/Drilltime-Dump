...
local _doorType, _distToDoor, allowedJobs, doorPin, item, allowedGang = "normal", 2, {}, "", "", ""

GetGangs = function()
    local result = promise:new()
    ESX.TriggerServerCallback('guille_doorlock:cb:getGangs', function(cb_result) 
...
RegisterNetEvent("guille_doorlock:client:setUpDoor", function()
    ...
    table['insert'](elements, {label = "Add Gang: ".. (Gangs[allowedGang] or ""), value = "addgang"})
    ...
    elements = elements, 
        ...
        elseif v == "addgang" then
            local elements = {}
            elements[#elements+1] = {label = "None", value = ""}
            for k,v in pairs(Gangs) do
                elements[#elements+1] = {label = k..' | '..v, value = k}
            end
            ESX['UI']['Menu']['Open']('default',GetCurrentResourceName(),"menu_door3",
            { 
            title = "Type menú", 
            align = "bottom-right", 
            elements = elements,
            }, function(data3, menu3)
                local v = data3['current']['value']
                allowedGang = v
                menu3['close']()
                ExecuteCommand("door")
        ...
        elseif v == "conf" then
            addDoor(_doorType, _distToDoor, allowedJobs, doorPin, item, allowedGang)
            ESX['UI']['Menu']['CloseAll']()
            _doorType = "normal"
            _distToDoor = 2
            doorPin = ""
...
addDoor = function(type, dist, jobs, pin, item, gang)
    dist = tonumber(dist)
    if not dist then
        dist = 2
    end
            ...
            while true do
                ...
                if IsEntityAnObject(entity) then
                    ...
                    if IsControlJustPressed(1, 38) then
                        ...
                        TriggerServerEvent("guille_doorlock:server:addDoor", _doorCoords, _doorModel, _heading, type, _textCoords, dist, jobs, pin, item, gang)
                ...
                if IsEntityAnObject(entity) then
                    ...
                    if IsControlJustPressed(1, 38) then
                        ...
                        if #_doorsDobule == 2 then
                            ...
                            entities = {}
                            TriggerServerEvent("guille_doorlock:server:addDoubleDoor", _doorsDobule, type, _textCoords, dist, jobs, pin, item, gang)
                            _doorsDobule = {}
                            break
                        else
...
local _selectedDoorJobs, _selectedDoorGang, pin, object = {}, nil, nil, nil
...
RegisterCommand("lockdoor", function()
    if show then
        ...
        if gang and tostring(gang.id) == _selectedDoorGang then
            _allowed = true
        end
        if _allowed then
            pulsed = true

...
CreateThread(function()
    ...
    while jobName == nil do Wait(10) end
    ...
    while (true) do
        ...
        if Config.Positions[jobName] then
            ...
            if (not uiOpen) then
                if ((playerVeh ~= 0) and (DoesEntityExist(playerVeh)) and (GetPedInVehicleSeat(playerVeh, -1) == playerPed)) then
                    ...
                    for k,v in pairs(Config.Positions[jobName].pos) do
                        local tempPos = v            
                        local tempDist = #(playerPos - vector3(tempPos.x, tempPos.y, tempPos.z))
                        local tempActionDist = Config.DefaultActionDistance
                ...
                elseif (customConfigPosIndex ~= nil) then
                    local tempPos = Config.Positions[jobName].pos[customConfigPosIndex]
                    local playerPos = GetEntityCoords(customVehicle)
                    local tempActionDist =  Config.DefaultActionDistance

                    local tempDist = GetDistanceBetweenCoords(playerPos.x, playerPos.y, playerPos.z, tempPos.x, tempPos.y, tempPos.z, true)
                    ...
                    elseif (menuOption.price > 0) then
                        ...
                        if (getCurrentMoney() >= tempPrice) then
                            ...
                            if (tempPrice > 0 or Config.AutoSaveVehiclePropertiesOnApply == true) then
                                ...
                                TriggerServerEvent('lls-mechanic:removeMoney', tempPrice, vehiclePorperties)
                            end
                        end
                    end
                end
...
CreateThread(function()
    for k,v in pairs(Config.Positions) do
     local blip = AddBlipForCoord(v.boss)

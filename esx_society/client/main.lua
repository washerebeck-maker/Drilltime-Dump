...
function OpenBossMenu(society, close, options)
	...
	ESX.TriggerServerCallback('esx_society:isBoss', function(isBoss)
		if isBoss then
			...
			if options.withdraw and not Config.LimitedJobs[society] then
				table.insert(elements, {label = _U('withdraw_society_money'), value = 'withdraw_society_money'})
			end

			if options.deposit and not Config.LimitedJobs[society] then
				table.insert(elements, {label = _U('deposit_society_money'), value = 'deposit_money'})
			end

			if options.wash  and not Config.LimitedJobs[society] then
			...
			if options.grades and not Config.LimitedJobs[society] then
		...
		for i=1, #employees, 1 do
			...
			table.insert(elements.rows, {
				...
				cols = {
					employees[i].name,
					gradeLabel,
					not Config.LimitedJobs[society] and '{{' .. _U('promote') .. '|promote}} {{' .. _U('fire') .. '|fire}}' 
					or '{{' .. _U('fire') .. '|fire}}'
				}
			})
		end
...
function OpenRecruitMenu(society)
	ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
		...
		for i=1, #players, 1 do
			local ped = GetPlayerPed(GetPlayerFromServerId(players[i].source))
		--	print (GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),GetEntityCoords(ped)))
			if players[i].job.name ~= society and GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),GetEntityCoords(ped))  <= 50.0 then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_' .. society, {
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_confirm_' .. society, {
				...
				if data2.current.value == 'yes' then
					...
					local can_hire = CheckIfBossCanHire(society)
					...
					if can_hire then 
						...
						ESX.ShowNotification('You have maximum number of workers !')
					end
...
CheckIfBossCanHire = function(society)
	local result = promise:new()
    ESX.TriggerServerCallback('esx_society:canHire', function(data) 
		result:resolve(data)  
    end, society)
